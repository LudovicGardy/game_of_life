...
class GameOfLife:
    ...
    def __init__(self, config: dict, no_window_boundaries: bool):
        ...
        self.rows = self.GRID_WIDTH // self.CELL_SIZE
        self.cols = self.GRID_HEIGHT // self.CELL_SIZE
...
class GameView:
    ...
    def __init__(self, game, config):
        pygame.init()
        self.screen = pygame.display.set_mode((game.GRID_WIDTH, game.GRID_HEIGHT))
        ...
        self.BLACK = tuple(colors['BLACK'])
        self.GRAY = tuple(colors['GRAY'])
        self.LIGHT_GRAY = tuple(colors['LIGHT_GRAY'])
    ...
    def draw_grid(self):
        ...
        for row in range(self.game.rows):
            for col in range(self.game.cols):
                color = self.BLACK if self.game.grid[row, col] == 1 else self.WHITE
        ...
        for x in range(0, self.game.GRID_WIDTH, self.game.CELL_SIZE):
            pygame.draw.line(self.screen, self.GRAY, (x, 0), (x, self.game.GRID_HEIGHT))
        for y in range(0, self.game.GRID_HEIGHT, self.game.CELL_SIZE):
            pygame.draw.line(self.screen, self.GRAY, (0, y), (self.game.GRID_WIDTH, y))
    ...
    def draw_instructions(self):
        ...
        y_offset = self.game.GRID_HEIGHT - len(instructions) * 20 - 10
    ...
    def prompt_for_pattern_name(self):
        ...
        while input_active:
            self.screen.fill(self.WHITE)
            prompt_text = self.font.render("Nom du pattern : ", True, self.BLACK)
            name_text = self.font.render(pattern_name, True, self.BLACK)
    ...
    def display_state_selection(self):
        ...
        menu_x, menu_y = (self.game.GRID_WIDTH - menu_width) // 2, (self.game.GRID_HEIGHT - menu_height) // 2
        menu_surface = pygame.Surface((menu_width, menu_height))
        menu_surface.fill(self.LIGHT_GRAY)
        ...
        while selecting:
            ...
            # Dessin du menu
            menu_surface.fill(self.LIGHT_GRAY)  # Effacer le menu pour redessiner
            for i in range(scroll_offset, min(len(saved_states), scroll_offset + max_visible_buttons)):
                rect = pygame.Rect(10, 10 + (i - scroll_offset) * (button_height + 10), menu_width - 20, button_height)
                pygame.draw.rect(menu_surface, self.HIGHLIGHT if rect.collidepoint(pygame.mouse.get_pos()[0] - menu_x,
                                                                            pygame.mouse.get_pos()[1] - menu_y) else self.GRAY, rect)
                text = self.font.render(saved_states[i], True, self.BLACK)
        ...
        while running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    col, row = x // self.game.CELL_SIZE, y // self.game.CELL_SIZE
